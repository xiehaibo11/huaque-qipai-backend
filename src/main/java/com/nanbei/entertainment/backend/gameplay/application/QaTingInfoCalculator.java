...
final class QaTingInfoCalculator {
    ...
    List<QaRoundTable.TingEntry> compute(List<Integer> hand) {
        return compute(hand, QaTaizhouJokerRule.synthetic(), BUDGET_NANOS);
    }

    /** 预算可注入的入口（超时降级路径的契约测试用）。 */
    List<QaRoundTable.TingEntry> compute(List<Integer> hand, long budgetNanos) {
        return compute(hand, QaTaizhouJokerRule.synthetic(), budgetNanos);
    }

    List<QaRoundTable.TingEntry> compute(
            List<Integer> hand, QaTaizhouJokerRule jokerRule, long budgetNanos) {
        long deadline = System.nanoTime() + budgetNanos;
        try {
            return computeWithin(hand, jokerRule, deadline);
    ...
    static List<QaRoundTable.TingEntry> computeWithin(List<Integer> hand, long deadlineNanos) {
        return computeWithin(hand, QaTaizhouJokerRule.synthetic(), deadlineNanos);
    }

    static List<QaRoundTable.TingEntry> computeWithin(
            List<Integer> hand, QaTaizhouJokerRule jokerRule, long deadlineNanos) {
